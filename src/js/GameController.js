...
export default class GameController {
  ...
  onLoadGame() {
    ...
    this.gameState.positionedCharacters = load.positionedCharacters
      .map((positionedCharacter) => {
        ...
        const { type, level, attack, defence, health, maxHealth } = positionedCharacter.character;
  ...
  attackCharacter(index, innerCharacter, callback) {
    const damage = Math.round(Math.max(
      this.selectedCharacter.character.attack - innerCharacter.character.defence, this.selectedCharacter.character.attack * 0.1,
    ));
  ...
  activateComputer() {
    ...
    // Cmputer character is trying to attack users character
  ...
  computerTeamUpdate() {
    ...
    const randomCharacters = generateCharacters(this.gameState.computer.allowedTypes, maxLevel, charactersCount);
